;;; -*- lexical-binding: t -*-
(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(package-selected-packages
   '(ace-window acme-theme astyle auto-hide cape cmake-mode cobol-mode
		corfu-prescient deft diff-hl diminish dumb-jump
		exec-path-from-shell form-feed-st geiser-chicken
		git-modes kind-icon marginalia nerd-icons-completion
		nerd-icons-dired nerd-icons-ibuffer ninja-mode
		no-littering org org-bullets org-modern
		paredit-everywhere paredit-menu side-notes srfi
		treesit-auto vertico visual-fill-column wgrep
		ws-butler)))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(compilation-error ((t (:background "gray80" :foreground "Red"))))
 '(flymake-error ((t (:underline (:color "Red" :style wave :position nil)))))
 '(font-lock-comment-face ((t (:foreground "#707070" :slant oblique))))
 '(hl-line ((t (:inherit highlight :extend t :background "LightGoldenrod2" :foreground "black")))))
